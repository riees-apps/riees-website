...
class Blog extends Component {
  ...
  filtro = event => {
    if (event.props.text.indexOf(`${this.state.value}`) !== -1 || event.props.title.indexOf(`${this.state.value}`) !== -1  )
      return true
    return false
  }
  render() {
    ...
    const openSearch = () => {
      this.setState({...this.state,search:!this.state.search,value:'',page:this.state.search === true && this.state.value !== '' ? 1 : this.state.page,initial: this.state.search === true && this.state.value !== '' ? 0 : this.state.initial,final: this.state.search === true && this.state.value !== ''  ? this.props.final : this.state.final })
    };
    ...
    const nextItems = event => {
      if (
        this.props.larger &&
        this.state.page !== Math.ceil(events.length !== 0 ? events.length  / this.props.final : 1)
      ) {
        document.documentElement.scrollTop = window.innerHeight * 2;
      }
      return event.slice(
    ...
    return (
      <div style={{ zIndex: "100", position: "relative" }}>
        ...
        <DivHeading className={this.props.larger ? "" : "displayNone"}>
          ...
          <Form>
          <input onChange={e => this.setState({ value: e.target.value,page:1,initial:0,final:this.props.final})} value={this.state.value} type="text" name="search" className={this.state.search ? 'openSearch' : 'closeSearch'} placeholder='Search by name...'/>
          <div onClick={() => openSearch()} className={this.state.search ? 'divIconSearch2' : 'divIconSearch'}>
          <i className={this.state.search ? 'fas fa-times iconSearch' : 'fas fa-search iconSearch2'}/>
          </div>
          </Form>
        ...
        <DivPosts side>
          ...
          />
          <h3 className={` ${events.length === 0 ? "" : "displayNone"}`}><FormattedMessage id="Result"/></h3>
          <span className={` ${events.length === 0 ? "" : "displayNone"}`}><FormattedMessage id="Result2"/> "{this.state.value}" <FormattedMessage id="Result3"/></span>
          <span className={` ${events.length === 0 ? "" : "displayNone"}`} ><FormattedMessage id="Result4"/></span>
